...
async function generateUrlString(idList) {
    ...
    await axios.get('https://libgen.is/json.php?ids='+urlString+'&fields=id,md5,openlibraryid')
...
async function addDownloadLink(completeList) {
    for (let obj of completeList) {
        ...
        var downloadUrl="http://62.182.86.140/main/"+downloadID+'/'+obj.md5+'/'
        if (obj.olid.length>3) {
            var coverUrl = 'https://covers.openlibrary.org/b/olid/'+ obj.olid +'-L.jpg'
        } else {
